<script lang='ts'>
	...
	$: statusColor = networkType === 'mainnet' ? 'bg-green-500' : networkType === 'testnet' ? 'bg-yellow-500' : 'bg-blue-500';
	$: textColor = networkType === 'mainnet' ? 'text-green-400' : networkType === 'testnet' ? 'text-yellow-400' : 'text-blue-400';
</script>
...
{#if showDetails}
	<div class="mt-2 text-xs text-gray-400">
		...
		<div>Explorer: <a href={config.explorerUrl} target="_blank" class="text-[#00ccba] hover:text-[#00eeda]">View</a></div>
	</div>
{/if}
