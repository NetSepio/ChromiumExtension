...
async function testDownloadSpeed() {
	...
	const urls = Array.from({ length: parallel }, () => `https://speed.cloudflare.com/__down?bytes=${fileSize}`);
	let totalBytes = 0;
...
async function testPing() {
	...
	try {
		...
		const variance = pings.reduce((sum, pingTime) => sum + Math.pow(pingTime - avgPing, 2), 0) / pings.length;
...
>
	...
	<div class="flex-1 flex flex-col space-y-4">
		...
		<div class="flex justify-center">
			<Speedometer speed={downloadSpeed} isTestRunning={isTestRunning} />
		</div>
