<script lang="ts">
	...
	async function loadBalance() {
		try {
			...
			console.log(`Current balance: ${balance} SOL on ${rpcUrl} network`);
		...
		{#if !showConfirmation}
			...
			<div class="space-y-5">
				...
				<button
					...
					class="flex w-full cursor-pointer items-center justify-center gap-2 rounded-3xl bg-gradient-to-r from-[#00ccba] to-[#00eeda] px-4 py-3 font-bold text-black shadow-lg transition-all hover:from-[#00eeda] hover:to-[#00ccba] hover:shadow-xl disabled:cursor-not-allowed disabled:from-gray-600 disabled:to-gray-600"
				>
					<Send size="16" />
					Review Transaction
				</button>
