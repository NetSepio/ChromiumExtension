...
<div>
	...
	<div class="mt-6 mx-auto">
		<a href="/Onboarding/import-old-wallet">
			<svg
				width="24"
				height="24"
				viewBox="0 0 24 24"
		...
		<h1 class="text-xl font-bold text-center mt-12 mb-3">Create Your Password</h1>
		<p class={`text-sm text-center mb-8 ${error !== '' ? 'text-red-500' : ''}`}>
			{error !== '' ? error : 'You will use this to unlock your wallet'}
		</p>
		<div>
			<label for="password" class="text-sm text-left mt-3 mb-1">New Password</label>
			<input name="password" type="password" class="secondary-input" bind:value={newPassword} />
		</div>
		<br />
		<div>
			<label for="confirmPassword" class="text-sm text-left mt-3 mb-1">Confirm Password</label>
			<input
		...
		<div class="form-control mt-16 mb-8">
			<label class="label justify-evenly cursor-pointer">
				<input
					type="checkbox"
				...
				<span class="text-sm dark:text-white">
					I agree to the
					<span class="text-[#11D9C5]">Terms of Service</span>
				</span>
			</label>
		</div>

		{#if termsAndConditions}
			<button on:click={handleSubmit} class="btn primary-button">create password</button>
		{:else}
			<button disabled class="btn primary-button">create password</button>
		{/if}

		<!-- =================First modal============== -->
		<div class="modal" class:modal-open={showModal}>
			<div class="modal-box py-16 dark:bg-[#222944] dark:text-white">
				<h1 class="text-2xl text-center font-bold mb-2">You are signing!</h1>

				<br />
				{#if data?.payload?.eula}
					<p class="text-sm text-center dark:text-green-100">
						{`${data?.payload?.eula}` ?? '...'}
					</p>
				{:else}
					<p class="text-sm text-center dark:text-green-100">cannot find message</p>
				{/if}

				<br />
				<div class="flex w-full mt-8 gap-4">
					<div class="grid flex-grow">
						<button class="btn secondary-button" on:click={() => (showModal = false)}>CANCEL</button
						>
					</div>

					<div class="grid flex-grow">
						<button class="btn primary-button" on:click={handleSave}> Sign </button>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="modal" class:modal-open={showSecondModal}>
			<div class="modal-box py-16 dark:bg-[#222944] dark:text-white">
				{#if error.length > 0}
					<h1 class="text-xl font-bold text-center mb-2">Unable to sign-in ☹️!</h1>
					<br />
					<p class="text-base text-center text-red-500">
						{error}
					</p>
					<br />
					<div class="flex w-full mt-2">
						<div class="grid flex-grow">
							<a href="/" class="btn secondary-button"> Try later </a>
						</div>
				...
				{:else}
					<div class="flex gap-4 justify-center items-center">
						<h1 class="text-3xl text-center mb-2">Congrats</h1>
						<svg
							width="68"
							height="66"
							viewBox="0 0 68 66"
					...
					<p class="text-sm text-center text-black dark:text-white">
						You are all set! Create your profile
					</p>
					<br />
					<div class="flex w-full mt-2">
						<div class="grid flex-grow">
							<a href="/create-profile">
								<button class="btn primary-button mx-auto"> go to profile </button>
							</a>
						</div>
					</div>
				{/if}
