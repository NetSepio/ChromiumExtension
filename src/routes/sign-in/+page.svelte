<script>
  ...
  function authenticator() {
    const authenticate = authenticateUser(password)
    return authenticate
  }
...
<section class="h-full p-8 bg-[#101212] text-white text-center capitalize relative grid">
  <h1 class="font-bold text-xl">Sign in to your account</h1>
  
  <div class="grid space-y-4">
    <form class="relative flex flex-col justify-center text-sm">
      <labeL class='text-left font-bold'>password</labeL>
      <input type={showPassword ? "text" : "password"} class="bg-[#3b3b3bbd] border-none outline-[#00887d] rounded-lg py-2 px-4 w-full placeholder:text-white/80" placeholder="Enter your password" bind:value={password} />
      {#if showPassword}
...
<Dialog open={showStatus} onClose={() => showStatus = false}>
  <div class='bg-[#1012128f] rounded-lg p-4 grid space-y-4 text-center'>
    ...
    >
      ...
      <defs>
   	...
   	<a class="self-end w-full rounded-3xl py-2 text-black cursor-pointer bg-gradient-to-b from-[#0b8f84] to-[#00ccba]" href='/' onclick={() => showStatus = false}>Go home</a>
  </div>
</Dialog>
